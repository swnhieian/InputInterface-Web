...
const Selector = (props) => {
    ...
    let items = data.map((item, no) => {
        ...
        let xpos = radius + radius/2 * Math.cos(angle) - width /2;
        let ypos = radius - radius/2 * Math.sin(angle);
        return (
    ...
    for (let i=0; i<data.length; i++) {
        ...
        lines.push(
            <div style={{height: '1px', backgroundColor: 'white', width: radius+'px', position:'absolute', left:radius+'px', top: radius+'px', transform: 'rotate('+angle+'rad)', transformOrigin: 'left'}}></div>
        );
    }
    return (
        <div style={{width: radius*2+'px',height: 2*radius+'px',borderRadius:'50%', backgroundColor:'green', position: 'relative', margin: 'auto'}}>
            {lines}
            {items}
        </div>
    );
